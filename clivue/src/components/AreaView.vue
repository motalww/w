...
export default {
  ...
  methods:{
    position(){
      this.$router.push('/maptest?name='+this.data.area)
    },
    load(id){
      this.request.get("http://localhost:8080/client/area/"+id).then(res=>{
        this.data=res.data
        if(this.data!=null){
          this.data.area=res.data[0].area
          this.data.image=res.data[0].image
          this.data.address=res.data[0].address
          this.data.introduction=res.data[0].introduction
        }
      })
    },
    loadComment(){
      this.request.get("/comment/area/"+this.areaId).then(res=>{
    ...
    send(){
      ...
      this.commentForm.areaId=this.areaId
