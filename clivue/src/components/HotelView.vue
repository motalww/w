  ...
  <div>
    ...
    <div style="margin-top: 5px">
      <mt-cell  title="地址:"  style="height: 50px">
        <span v-model="data.address" style="margin-right: 40px">{{data.address}}</span>
...
export default {
  ...
  created() {
    this.load(this.hotelId)
     this.loadComment(this.hotelId)
  },
  methods:{
    load(id){
      this.request.get("http://localhost:8080/client/hotel/"+id).then(res=>{
        this.data=res.data
        if(this.data!=null){
    ...
    loadComment(){
      this.request.get("/comment/hotel/"+this.hotelId).then(res=>{
    ...
    send(){
      ...
      this.commentForm.hotelId=this.hotelId
